...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Inicialização do Sortable para permitir drag and drop
  const tbody = document.querySelector('#produtos tbody');
  new Sortable(tbody, {
    animation: 150,
    handle: 'td:not(:last-child)', // Permite arrastar por qualquer célula exceto a última (botão remover)
    onEnd: function() {
      salvarLista(); // Salva a nova ordem no localStorage
      mostrarNotificacao('Item reordenado com sucesso');
    }
  });
});
